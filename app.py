...
with tabs[1]:
    ...
    if not POSE_AVAILABLE:
        st.warning("⚠️ Pose detection requires a webcam and is only available when running locally on your computer.")
        st.info("💡 To use this feature: run `streamlit run app.py` on your local machine. All other features work here on the cloud!")
    else:
...
with tabs[2]:
    ...
    history = pd.DataFrame({"session": [1, 2, 3, 4, 5], "form_score": [60, 70, 78, 85, 92]})
